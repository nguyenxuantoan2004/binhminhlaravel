...
@section('title', 'Trang Xe')

@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                <h5 class="">Danh sách xe</h5>
            ...
            <div class="card-body">
                <div class="analytic">
                    ...
                    <a href="{{ route('motorbike.index', ['status' => 'pending', 'search' => request('search')]) }}"
                        class="text-primary text-decoration-none {{ $status == 'pending' ? 'fw-bolder' : '' }}">Chờ
                        duyệt<span class="text-muted">({{ $counts['pending'] }})</span></a> |

                    <a href="{{ route('motorbike.index', ['status' => 'draft', 'search' => request('search')]) }}"
                        class="text-primary text-decoration-none {{ $status == 'draft' ? 'fw-bolder' : '' }}">Nháp<span
                            class="text-muted">({{ $counts['draft'] }})</span></a> |

                    <a href="{{ route('motorbike.index', ['status' => 'maintenance', 'search' => request('search')]) }}"
                        class="text-primary text-decoration-none {{ $status == 'maintenance' ? 'fw-bolder' : '' }}">Bảo
                        trì<span class="text-muted">({{ $counts['maintenance'] }})</span></a> |

                    <a href="{{ route('motorbike.index', ['status' => 'trash', 'search' => request('search')]) }}"
                ...
                <form action="" method="post" id="action-form" data-name-module="{{ $nameModule }}"
                    ...
                    <table class="table table-striped table-checkall">
                        ...
                        <tbody>
                            @if ($motorbikes->isNotEmpty())
                                ...
                                @foreach ($motorbikes as $motorbike)
                                    ...
                                    <tr class="align-middle">
                                        <td>
                                            <input type="checkbox" name="selected" value="{{ $motorbike->id }}">
                                        </td>
                                        <td>{{ $i }}</td>
                                        <td>
                                            <a href="{{ route('motorbike.edit', ['motorbike' => $motorbike->id]) }}"
                                                class="text-decoration-none text-secondary">{{ $motorbike->name }}</a>
                                        </td>
                                        <td>
                                            {{ $motorbike->quantity }}
                                        </td>
                                        <td>
                                            {{ $motorbike->manufacture_year }}
                                        </td>
                                        <td>
                                            {{ $motorbike->color }}
                                        </td>
                                        <td>
                                            {{ $motorbike->vehicle_condition }}
                                        </td>
                                        <td>
                                            {{ $motorbike->categoryMotorbike->name }}
                                        </td>
                                        <td>
                                            {{ format_price($motorbike->rental_price) }}
                                        </td>
                                        {{-- Accessors --}}
                                        @if ($status != 'trash')
                                            <td><span
                                                    class="{{ $motorbike->class }}">{{ $motorbike->status_label }}</span>
                                            </td>
                                        @endif
                                        <td>{{ $motorbike->created_at }}</td>
                                        <td>
                                            @if ($status == 'trash')
                                                <button class="btn btn-success btn-sm rounded-0 text-white btn-restore"
                                                    data-toggle="tooltip" title="Khôi phục"
                                                    data-id="{{ $motorbike->id }}" data-name="{{ $motorbike->name }}"
                            ...
                            @else
                                <tr>
                                    <td colspan="12">Không tồn tại dữ liệu</td>
                                </tr>
                            @endif
                ...
                {{ $motorbikes->appends(['status' => request('status'), 'search' => request('search')])->onEachSide(1)->links() }}
