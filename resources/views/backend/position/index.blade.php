...
@section('title', 'Trang Chức Vụ')

@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                <h5 class="">Danh sách chức vụ</h5>
            ...
            <div class="card-body">
                            ...
                            @if ($positions->isNotEmpty())
                                ...
                                @foreach ($positions as $position)
                                    ...
                                    <tr class="align-middle">
                                        <td>
                                            <input type="checkbox" name="selected" value="{{ $position->id }}">
                                        </td>
                                        <td>{{ $i }}</td>
                                        <td>
                                            <a href="{{ route('position.edit', ['position' => $position->id]) }}"
                                                class="text-decoration-none text-secondary">{{ $position->name }}</a>
                                        </td>
                                        {{-- Accessors --}}
                                        @if ($status != 'trash')
                                            <td><span class="{{ $position->class }}">{{ $position->status_label }}</span>
                                            </td>
                                        @endif
                                        ...
                                        <td>
                                            @if ($status == 'trash')
                                                <button class="btn btn-success btn-sm rounded-0 text-white btn-restore"
                                                    data-toggle="tooltip" title="Khôi phục" data-id="{{ $position->id }}"
                ...
                {{ $positions->appends(['status' => request('status'), 'search' => request('search')])->onEachSide(1)->links() }}
