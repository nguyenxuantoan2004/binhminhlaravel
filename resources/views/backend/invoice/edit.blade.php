...
@section('title', 'Trang Cập Nhật Đơn Đặt Xe')

@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold">
                <h5>Cập nhật đơn đặt xe</h5>
            </div>
            ...
            <div class="card-body">
                <form action="{{ route('invoice.update', ['invoice' => $invoice->id]) }}" method="POST" id="orderForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="datetime-local" name="start_date" id="start_date" class="form-control"
                                value="{{ old('start_date', $invoice->start_date->format('Y-m-d\TH:i')) }}">
                            @error('start_date')
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="expected_return_date" class="form-label">Ngày trả thực tế</label>
                    ...
                    <div class="row">
                            ...
                            <select name="motorbike_id" id="motorbike_id" class="form-control">
                                ...
                                @foreach ($motorbikes as $motorbike)
                                    <option value="{{ $motorbike->id }}" data-price="{{ $motorbike->rental_price }}"
                                        {{ old('motorbike_id', $invoice->motorbike_id) == $motorbike->id ? 'selected' : '' }}>
                                        {{ $motorbike->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" id="rental_price" class="form-control" readonly
                                value="{{ old('rental_price', $invoice->motorbike->rental_price ?? 0) }}">
                            @error('rental_price')
                                <div class="text-danger">{!! $message !!}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="motorbike_receipt_method" id="motorbike_receipt_method" class="form-control">
                                <option value="store"
                                    {{ old('motorbike_receipt_method', $invoice->motorbike_receipt_method) == 'store' ? 'selected' : '' }}>
                                    Nhận tại cửa hàng</option>
                                <option value="delivery"
                                    {{ old('motorbike_receipt_method', $invoice->motorbike_receipt_method) == 'delivery' ? 'selected' : '' }}>
                                    Giao tận nơi</option>
                            </select>
                        ...
                        <div class="col-md-6 mb-3" id="branch_selection"
                            style="{{ $invoice->motorbike_receipt_method == 'store' ? '' : 'display: none;' }}">
                        ...
                        <div class="col-md-6 mb-3" id="pickup_location"
                            style="{{ $invoice->motorbike_receipt_method == 'delivery' ? '' : 'display: none;' }}">
                            ...
                            <input type="text" name="motorbike_pickup_location" id="motorbike_pickup_location"
                                class="form-control"
                                value="{{ old('motorbike_pickup_location', $invoice->motorbike_pickup_location) }}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3" id="additional_fees_row">
                            ...
                            <input type="number" name="additional_fees" id="additional_fees" class="form-control"
                                value="{{ old('additional_fees', $invoice->additional_fees ?? 0) }}" readonly>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" id="total_rental_duration" name="total_rental_duration"
                                ...
                                value="{{ old('total_rental_duration', $invoice->total_rental_duration ?? '0 ngày 0 giờ') }}">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="vat_fee" id="vat_fee" class="form-control"
                                value="{{ old('vat_fee', $invoice->vat_fee ?? 0) }}">
                            @error('vat_fee')
                                <div class="text-danger">{!! $message !!}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" id="total_cost" name="total_cost" class="form-control" readonly
                                value="{{ old('total_cost', $invoice->total_cost ?? 0) }}">
                            @error('total_cost')
                                <div class="text-danger">{!! $message !!}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status" class="form-control">
                                <option value="pending"
                                    {{ old('status', $invoice->status) == 'pending' ? 'selected' : '' }}>Chờ xác nhận
                                </option>
                                <option value="confirmed"
                                    {{ old('status', $invoice->status) == 'confirmed' ? 'selected' : '' }}>Đã xác nhận
                                </option>
                                <option value="delivering"
                                    {{ old('status', $invoice->status) == 'delivering' ? 'selected' : '' }}>Đang giao
                                    xe</option>
                                <option value="waiting_for_pickup"
                                    {{ old('status', $invoice->status) == 'waiting_for_pickup' ? 'selected' : '' }}>Chờ
                                    nhận xe</option>
                                <option value="picked_up"
                                    {{ old('status', $invoice->status) == 'picked_up' ? 'selected' : '' }}>Đã nhận xe
                                </option>
                                <option value="completed"
                                    {{ old('status', $invoice->status) == 'completed' ? 'selected' : '' }}>Hoàn thành
                                </option>
                                <option value="cancelled"
                                    {{ old('status', $invoice->status) == 'cancelled' ? 'selected' : '' }}>Đã hủy
                                </option>
                            </select>
                            @error('status')
                                <div class="text-danger">{!! $message !!}</div>
                        ...
                        <div class="col-12">
                            <label for="after_rental_status" class="form-label">Trạng thái khi trả xe</label>
                            <textarea name="after_rental_status" id="after_rental_status" cols="30" rows="5" class="form-control">{{ old('after_rental_status', $invoice->after_rental_status) }}</textarea>
                        </div>
                    </div>
                    <br>
                    <button type="submit" class="btn btn-primary">Cập nhật đơn đặt xe</button>
                </form>
            </div>
        </div>
    </div>
