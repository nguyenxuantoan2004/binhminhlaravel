...
@section('title', 'Trang Loại Xe')

@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                <h5 class="">Danh sách loại xe</h5>
            ...
            <div class="card-body">
                            ...
                            @if ($categorys->isNotEmpty())
                                ...
                                @foreach ($categorys as $category)
                                    ...
                                    <tr class="align-middle">
                                        ...
                                        <td>{{ $i }}</td>
                                        <td>
                                            <a href="{{ route('motorbike-category.edit', ['motorbike_category' => $category->id]) }}"
                                                class="text-decoration-none text-secondary">{{ $category->name }}</a>
                                        </td>
                                        {{-- Accessors --}}
                                        @if ($status != 'trash')
                                            <td><span class="{{ $category->class }}">{{ $category->status_label }}</span>
                                            </td>
                                        @endif
                                        ...
                                        <td>
                                            @if ($status == 'trash')
                                                <button class="btn btn-success btn-sm rounded-0 text-white btn-restore"
                                                    data-toggle="tooltip" title="Khôi phục" data-id="{{ $category->id }}"
                                            ...
                                            @else
                                                ...
                                                {{-- <form
                                            action="{{ route('motorbike-category.destroy', ['motorbike_category' => $category->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                ...
                {{ $categorys->appends(['status' => request('status'), 'search' => request('search')])->onEachSide(1)->links() }}
            </div>
        </div>
    </div>
@endsection
